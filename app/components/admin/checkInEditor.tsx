...
export default function CheckInEditor() {
  ...
  return <section className="min-w-0 space-y-5 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-6">
    ...
    <div className="grid grid-cols-3 gap-2 text-center">{[["Undangan hadir", checked.length], ["Orang hadir", checked.reduce((sum, row) => sum + (row.partySize || 0), 0)], ["Belum hadir", realGuests.length - checked.length]].map(([label, count]) => <div key={label} className="rounded-xl bg-stone-100 px-2 py-4"><strong className="block text-2xl tabular-nums text-emerald-800">{loaded ? count : "-"}</strong><span className="mt-1 block text-[11px] leading-4 text-stone-600">{label}</span></div>)}</div>
    ...
    <div className="flex items-center justify-between gap-3"><h3 className="font-semibold">Daftar tamu</h3><button disabled={busy} onClick={() => refresh().then(() => setNotice("Rekap diperbarui.")).catch(error => setNotice(error.message))} className={`${button} text-emerald-800`}>Perbarui rekap</button></div>
    ...
    <div className="max-h-[32rem] space-y-2 overflow-y-auto">{filtered.map(row => <div key={row.token} className="flex items-center gap-3 rounded-xl border border-stone-200 p-3"><div className="min-w-0 flex-1"><p className="break-words text-sm font-semibold">{row.name}{!!row.isTest && <span className="ml-2 text-xs text-amber-700">(Tes)</span>}</p><p className={`mt-1 text-xs ${row.checkedInAt ? "text-emerald-700" : "text-stone-500"}`}>{row.checkedInAt ? `${row.partySize} orang hadir` : "Belum hadir"}</p>{row.checkedInAt && <p className="mt-1 text-[11px] text-stone-500">{new Date(row.checkedInAt).toLocaleString("id-ID")}</p>}</div>{row.checkedInAt ? <span className="shrink-0 rounded-full bg-emerald-50 px-3 py-2 text-xs font-medium text-emerald-700">Sudah hadir</span> : <button disabled={busy} onClick={() => lookup(row.token)} className={`${button} shrink-0 border-emerald-200 bg-emerald-50 text-emerald-800`}>Check-in</button>}</div>)}{loaded && !filtered.length && <p className="p-3 text-sm text-stone-500">{rows.length ? "Nama tidak ditemukan." : "Belum ada tamu. Hubungi admin utama untuk menambahkan daftar tamu."}</p>}</div>
  </section>;
}
