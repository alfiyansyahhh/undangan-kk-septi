...
function PhotoPicker({ label, value, photos, onChange, multiple = false }: { label: string; value: string[]; photos: GDrivePhoto[]; onChange: (value: string[]) => void; multiple?: boolean }) {
  ...
  const moveItem = (fromIndex: number, toIndex: number) => {
    if (!multiple || fromIndex === toIndex || fromIndex < 0 || toIndex < 0 || fromIndex >= value.length || toIndex >= value.length) return;
    const next = [...value];
  ...
  return <div className="min-w-0 self-start rounded-lg border border-stone-200 p-3 space-y-3">
    ...
    <div className="flex gap-2 overflow-x-auto pb-1" style={{ scrollbarWidth: "thin", scrollbarColor: "#c9a96e transparent" }}>
      {value.map((src, index) => <div key={`${src}-${index}`} className="shrink-0 space-y-1">
        ...
        <div className="flex flex-wrap gap-1 text-[10px]">
          {multiple && <button type="button" title="Naikkan urutan" disabled={index === 0} className="rounded border border-stone-300 px-1.5 py-0.5 disabled:opacity-40" onClick={() => moveItem(index, index - 1)}>↑</button>}
          {multiple && <button type="button" title="Turunkan urutan" disabled={index === value.length - 1} className="rounded border border-stone-300 px-1.5 py-0.5 disabled:opacity-40" onClick={() => moveItem(index, index + 1)}>↓</button>}
          <button type="button" aria-label="Hapus foto" className="rounded border border-stone-300 px-1.5 py-0.5" onClick={() => onChange(value.filter((_, i) => i !== index))}>Hapus</button>
        </div>
      </div>)}
    ...
    {open && <div className="space-y-3"><input aria-label={`Cari foto ${label}`} placeholder="Cari nama foto…" value={query} onChange={e => { setQuery(e.target.value); setLimit(30); }} className={input} />
      <div className="grid grid-cols-3 sm:grid-cols-6 gap-2 max-h-80 overflow-y-auto pb-1" style={{ scrollbarWidth: "thin", scrollbarColor: "#c9a96e transparent" }}>
  ...
  return <div className="space-y-6">
    <section className="rounded-xl bg-white border p-5 space-y-4"><h2 className="font-bold">Foto per section</h2><p className="text-sm text-stone-500">Pilih dari pustaka foto. Foto tetap tersedia untuk galeri; pilihan section disimpan terpisah. Untuk slideshow, urutan pilihan menjadi urutan tampil.</p>
      <div className="grid items-start md:grid-cols-2 gap-4">
        ...
        {([['akad', 'Foto akad'], ['resepsi', 'Foto resepsi'], ['gift', 'Background hadiah'], ['galleryCover', 'Foto utama galeri'], ['closing', 'Foto penutup']] as const).map(([key, label]) => <PhotoPicker key={key} label={label} photos={photos} value={data.photos[key] ? [data.photos[key]!] : []} onChange={value => onChange({ ...data, photos: { ...data.photos, [key]: value[0] || '' } })} />)}
      </div>
    </section>
