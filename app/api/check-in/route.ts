...
export async function GET(request: Request) {
  ...
  const rows = await database().prepare("SELECT g.token, g.name, c.checked_in_at AS checkedInAt, c.party_size AS partySize FROM guests g LEFT JOIN check_ins c ON c.guest_token = g.token ORDER BY c.checked_in_at DESC, g.name").all();
  return json(rows);
}
